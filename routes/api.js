...
router.post('/contact', async (req, res) => {
  try {
    ...
    await run('INSERT INTO messages (name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?)',
...
router.get('/services', async (req, res) => {
  try {
    ...
    await getDb();
    const services = await query('SELECT * FROM services WHERE active = 1 ORDER BY order_index ASC');
    res.json(services);
...
router.get('/portfolio', async (req, res) => {
  try {
    const { getDb, query } = require('../database/db');
    await getDb();
    const items = await query('SELECT * FROM portfolio WHERE active = 1 ORDER BY order_index ASC');
    res.json(items);
  } catch (err) { res.status(500).json([]); }
});
...
router.get('/testimonials', async (req, res) => {
  try {
    const { getDb, query } = require('../database/db');
    await getDb();
    const items = await query('SELECT * FROM testimonials WHERE active = 1 ORDER BY id DESC');
    res.json(items);
  } catch (err) { res.status(500).json([]); }
});
